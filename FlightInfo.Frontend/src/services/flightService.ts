...
export interface Reservation {
    ...
    createdAt: string;
    cancelledAt?: string;
    // Backend'den gelen doğrudan alanlar
    flightNumber?: string;
    origin?: string;
    destination?: string;
    departureTime?: string;
    ...
    totalPrice?: number;
    currency?: string;
    // Join edilen veriler (artık kullanılmıyor)
    flight?: Flight;
    user?: User;
}
...
// Rezervasyon oluştur (havacılık standartlarına uygun basit API)
export async function createReservation(
    ...
    console.log('REZERVASYON DEBUG: API request data:', requestData);

    const response = await api.post<Reservation>("/Reservation", requestData);
    return response.data;
}
...
// Rezervasyonları flight bilgileriyle birlikte getir (backend'den hazır gelir)
export async function getUserReservationsWithFlights(): Promise<Reservation[]> {
    const response = await api.get<Reservation[]>("/Reservation");
    console.log("Rezervasyon verileri:", response.data);
    return response.data;
}
...
export async function restoreReservation(reservationId: number): Promise<Reservation> {
    const response = await api.post<Reservation>(`/Reservation/${reservationId}/restore`);
    return response.data;
}
