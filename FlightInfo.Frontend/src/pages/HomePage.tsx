...
function HomePage() {
    ...
    const normalizeDateInput = (raw: string): { iso?: string; error?: string; incomplete?: boolean } => {
        if (!raw) return { iso: "" };
        const isoPattern = /^\d{4}-\d{2}-\d{2}$/;
        if (isoPattern.test(raw)) return { iso: raw };
        const trPattern = /^(\d{2})\.(\d{2})\.(\d{4})$/;
        if (trPattern.test(raw)) {
            const [, dd, mm, yyyy] = raw.match(trPattern)!;
            return { iso: `${yyyy}-${mm}-${dd}` };
        }
        if (raw.length < 4) return { incomplete: true };
        if (raw.includes(".") && raw.length < 10) return { incomplete: true };
        if (raw.includes("-") && raw.length < 10) return { incomplete: true };
        return { error: "Tarih GG.AA.YYYY veya YYYY-AA-GG olmalı" };
    };

    const handleDateBlur = (field: "departureDate" | "returnDate", value: string) => {
        // Blur'da aynı doğrulamayı uygula; toast yok
    ...
    const handleDateChange = (field: "departureDate" | "returnDate", value: string) => {
        ...
        const { iso, error, incomplete } = normalizeDateInput(value);
        ...
        if (!isValid) {
            setFieldError("Geçersiz tarih. Lütfen kontrol edin.");
            setSearchData({ ...searchData, [field]: value });
            return;
        }
        ...
        if (year < 2024 || year > 2070) {
            setFieldError("Sadece 2024 ile 2070 arasındaki tarihler geçerlidir.");
            setSearchData({ ...searchData, [field]: iso! });
            return;
        }
    ...
    const [dateErrors, setDateErrors] = useState<{ departureDate?: string; returnDate?: string }>({});
                        ...
                        <div className="form-row">
                            <div className="form-group">
                                ...
                                <input
                                    ...
                                    min="2024-01-01"
                                    max="2070-12-31"
                                    onBlur={(e) => handleDateBlur("departureDate", e.target.value)} // 🔥 kontrol sadece blur’da
                                    onChange={(e) => handleDateChange("departureDate", e.target.value)}
                                />
                                {dateErrors.departureDate && (
                                    <div className="field-error">{dateErrors.departureDate}</div>
                                )}
                            </div>
                            ...
                            {searchData.tripType === "roundtrip" && (
                                <div className="form-group">
                                    ...
                                    max="2070-12-31"
                                        ...
                                        onInvalid={(e) => e.preventDefault()} // 🔒 yine engelle
                                    />
                                {dateErrors.returnDate && (
                                    <div className="field-error">{dateErrors.returnDate}</div>
                                )}
                                </div>
                            )}
